...
class GPUModelRunner:
    ...
    def initialize_kv_cache(self, num_blocks: int) -> None:
        ...
        sliding_window_layers = self.model_config.get_sliding_window_layers(
            self.parallel_config)
        for layer_idx in range(self.num_attn_layers):
            cache_shape = kv_cache_shape
            if layer_idx in sliding_window_layers:
                num_swa_key_value_heads = self.model_config.get_head_size_swa()
                head_size_swa = self.model_config.get_num_swa_key_value_heads(self.parallel_config)
                cache_shape = FlashAttentionBackend.get_kv_cache_shape(
                    num_blocks, self.block_size, num_swa_key_value_heads,
                    head_size_swa)
